...
void MedService::exportHTML(const string& fileName) const
{
    ...
    for (const auto& med : recipe)
    {
        out << "<p>"<< med->get_name()<< " "<< med->get_price()<< " "<< med->get_producer()<< " "<< med->get_active_substance()<< "</p>";
    }
    out << "</body></html>";
}
...
map<string, ProducerDTO> MedService::reportByProducer() const
{
    map<string, ProducerDTO> report;

    for (const auto& med : repo.getAll())
    {
        const string& producer = med.get_producer();

        auto it = report.find(producer);

        if (it == report.end())
        {
            report[producer] = ProducerDTO(producer, 1);
        }
        else
        {
            report[producer] = ProducerDTO(producer, it->second.getCount() + 1);
        }
    }
    return report;}
