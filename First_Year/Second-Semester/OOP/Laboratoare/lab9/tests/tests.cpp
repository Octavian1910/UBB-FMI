...
void testRepostiory()
{
    ...
    assert(rez.size() == static_cast<size_t>(repo.size()) && repo.size() == 5);
...
void testService()
{
    ...
    assert(static_cast<size_t>(list.size()) == static_cast<size_t>(service.size())17);
...
void test_report()
{
    ...
    serv.store("Nurofen", 20, "Bayer", "Ibuprofen");
    serv.store("Aspirina", 15, "Bayer", "Acid");
    serv.store("Aulin", 30, "Terapia", "Nimesulid");

    const auto& report = serv.reportByProducer();

    assert(report.size() == 2);

    assert(report.at("Bayer").getProducer() == "Bayer");
    assert(report.at("Bayer").getCount() == 2);

    assert(report.at("Terapia").getProducer() == "Terapia");
    assert(report.at("Terapia").getCount() == 1);
}
